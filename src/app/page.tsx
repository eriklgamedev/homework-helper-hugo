...
function TaskCard({
  ...
  return (
    <div
      className="flex items-center gap-3 rounded-2xl bg-white p-4 shadow-sm border border-gray-100 transition-transform active:scale-[0.985]"
      style={{
        animation: `slideUp 0.4s cubic-bezier(0.16, 1, 0.3, 1) ${index * 0.04}s both`,
    ...
    setTasks((prev) =>
      prev.map((task) => {
        ...
        if (completing) {
          ...
          setTimeout(
            () => setBouncingIds((s) => { const n = new Set(s); n.delete(id); return n; }),
            500
          );
        }
